...
from app.services.conversion import get_secured_exchange_rate, get_customer_quote, SUPPORTED_PAYMENT_CURRENCIES
...
async def create_session(payload: SessionPayload, user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    rate_data = get_secured_exchange_rate(pair, spread_percentage=0)
...
async def pay_session(session_id: UUID4, payload: PaymentPayload, db: AsyncSession = Depends(get_db)):
    ...
    customer_quote = get_customer_quote(session.fiat_amount, session.fiat_currency, payload.payment_currency)
    if not customer_quote:
        raise InvalidCurrencyException(payload.payment_currency)
...
async def get_customer_payment_quote(session_id: UUID4, payment_currency: str, db: AsyncSession = Depends(get_db)):
    ...
    if not session:
        raise PaymentProcessingException("Checkout session not found.")
    quote = get_customer_quote(session.fiat_amount, session.fiat_currency, payment_currency)
    if not quote:
        raise InvalidCurrencyException(payment_currency)
